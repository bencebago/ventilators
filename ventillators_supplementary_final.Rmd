...
setwd("C:/Users/Bence/OneDrive/Documents/research/ventillators")
load("C:/Users/Bence/OneDrive/Documents/research/ventillators/Main_data_MM_corrected.rdata")
d$country=factor(d$country)
Main_MM <- d
load("C:/Users/Bence/OneDrive/Documents/research/ventillators/Main_data_Yougov.rdata")
Main_Yougov <- d.demo.common
Main_Yougov=subset(Main_Yougov, select=c(-subset, -features))
load("C:/Users/Bence/OneDrive/Documents/research/ventillators/Main_data_Yougov_without_pay_in_features.rdata")
d=subset(d, select=c(id, features, subset, country, question))
Main_Yougov=merge(Main_Yougov, d, by=c("id", "country", "question"))

load("C:/Users/Bence/OneDrive/Documents/research/ventillators/Subject_data_MM.rdata")
Main_MM =merge(Main_MM, Subject_Information_MM, by=c("id"))
load("C:/Users/Bence/OneDrive/Documents/research/ventillators/MM_with_pay.rdata")
d=subset(d, select=c("id", "Pay", "question"))
Main_MM =merge(Main_MM, d, by=c("id", "question"))
load("C:/Users/Bence/OneDrive/Documents/research/ventillators/US_specific_data.Yougov.rdata")
Us_Yougov <- d.demo.us
rm(list=c("d", "d.demo.common", "d.demo.us", "Subject_Information_MM"))
   
France_Yougov <- read.csv("Frenc_specific_Yougov.csv", header=T, sep=";", stringsAsFactors = T)
France_Yougov <- subset(France_Yougov, country=="FR")
...
Us_Yougov$gender=ifelse(Us_Yougov$gender=="Male", 0.5, -0.5)
...
model<-glm(Us_Yougov[Us_Yougov$question=="Allocations", i] ~ gender + patient+ age + smoker + health2 + political + education + race_re +religious, data=Us_Yougov[Us_Yougov$question=="Allocations", ], family=family,na.action=na.omit)
f=broom.mixed::tidy(model)
f$estimate=format(round(f$estimate,2), digits=2)
f$estimate=apply_if(f$estimate, f$p.value<0.05, function(x) paste0(x, "*") )
f$estimate=apply_if(f$estimate, f$p.value<0.01, function(x) paste0(x, "*") )
...
model<-glm(Us_Yougov[Us_Yougov$question=="Re-allocations", i] ~ gender + patient+ age + smoker + health2 + political + education + race_re +religious, data=Us_Yougov[Us_Yougov$question=="Re-allocations", ], family=family,na.action=na.omit)
f=broom.mixed::tidy(model)
f$estimate=format(round(f$estimate,2), digits=2)
f$estimate=apply_if(f$estimate, f$p.value<0.05, function(x) paste0(x, "*") )
f$estimate=apply_if(f$estimate, f$p.value<0.01, function(x) paste0(x, "*") )

---
title: "Supplementary Appendix for 'Policies for allocating scarce medical resources will face universal citizen polarization'"
...
##**1.2 Ethics**
Max Planck Institute for Human Development, and the University of British Columbia Ethical committees both approved the experiment a priori. Participants were presented with an informed consent at the beginning of the experiment, in which they were informed about the general objective of the research, the duration of the survey, that their IP will be collected but will not be used or made publicly available, that they will receive compensation [only in YouGov survey]. In the YouGov survey, the consent form was presented on a separate page and people who did not consent could not participate. In the MM experiment, the consent form was presented on the same page as the rest of the survey, but participants could tick a box to signal that they wanted to opt out the experiment. In this case, their data was not saved. In the Moral Machine data, IP addresses were collected and are stored in the ATLAS MongoDB non-public server, and only one author (E. Awad) has access to them. These were only used to identify the country the participant was from. In the YouGov survey, IP addresses were collected and are saved on YouGov servers to which YouGov has sole access to and in no way they can be paired with the rest og the data.

##**1.3 Survey methodology**

###**1.3.1 Data collection procedures**

The YouGov survey was a closed one. Initial contact with participants was made through the internet, and YouGov specifically targeted and recruited participants from its panelists. One important thing to note is that in Japan and Brazil, the sampling system was set up differently than in the US and France. In Japan and Brazil, respondents were sent generic invites and were sent into the first available survey, while in the US and France, respondents were sent specific invites to this survey. Participants filled out an online survey that automatically captured their responses. The survey was entirely voluntary. The main part of the questionnaire was distributed over two pages: participants first responded to the "allocation" and later to the "re-allocation" questions (8 and 7 items per page, respectively). Within one page, items were presented in a randomized order. The demographic questions were presented on different screens, after the "re-allocation" items. Participants were given the possibility to go back with a "Back" button and change their responses. There were no review step, once participants submitted their response, they were not able to change them any more. The survey required completeness, i.e., participants could not submit it without responding to every single question. All participants had a unique YouGov ID with which we defined unique visitors. The view rate (invited respondents that clicked on the survey link) was 59.03% in the US, 53.85% in France, and approximately 15-25% in Japan and Brazil. The completition rate (people who clicked on the link and finished it) was 91.43% in the US, 92.02% in France, 67.38% in Japan, 84.89% in Brazil. We did not record partial data and which point of the survey a given participant quit, therefore, participation rate (visitors who agreed to participate but did not finish the survey) cannot be calculated. No Cookies were used, no duplicate entries were  possible since all respondents had a unique identifier. Once people filled in the survey, they were not able to log in and provide an answer any more. Participants who did not finish the questionnaire could not submit their response, and hence, their response was nor saved nor analysed. 


The MM was an open survey. Initial contact with participants was made through the internet, specifically, through the Moral Machine website (moralmachine.org). Participants were recruited through this website, but the survey was not advertised in any other way. The website is home for multiple surveys dealing with ethical decision making in AI systems (for example, ethical decision making of self-driving vehicules). The survey was completely voluntary. The survey was presented on one page, with all items. On the top of the page the consent form was presented and then the "allocation" questions (8 items) in randomized order. After, the "re-allocation" items (7 items) were presented in a randomized order. The demographic questions were presented on the same screen as the rest of the questions. Participants could have changed their responses to all questions at any time, until they submitted their response. Participants were not able to review their responses afterwards. Browser fingerprints  were used to define unique visitors. Users were allowed to submit many times, but browser fingerprints were used to identify and exclude duplicates (see details under "data pre-processing"). There were no completeness check, participants could have submitted their survey without answering all the questions. Data from these participants were also saved and analysed. Over the survey period, 250 000 unique visitors visited the moralmachine.org website, out of which 9951 clicked and completed this survey. The accurate view rate cannot be calculated because we did not collect information on how many people simply clicked on the survey. Data of those people who did not click on the "Submit" button at the end of the survey were not saved in any way. For the same reason, completition rate cannot be calculated either. Since all questions were on the same page, participation rate is not meaningful. IP addresses were only used to identify the country the participant is from, and are not part of the final (openly accessible) dataset.


Before the data collection was started survey functionality was thoruoghly tested. 

###**1.3.2 Sampling strategy and data pre-processing**
...
and a four-way stratification of gender, age (4-categories), race (4-categories), and education (4-categories), to produce the final weight. This was all done by YouGov, who, in the end, shared the final dataset of 4000 people with us. In all 4 countries, participants received money for participation:  $0.5 in the USA; 0.5 Euro in France, 50 Yen in Japan and 1-2 Brazilian reals in Brazil.
...
The Moral Machine (MM) data went through pre-screening during which we excluded a number of entries. The original data included 9951 records. First, out of the 9951 submissions, 290 opted out of sharing their data (~3%). Of the remaining 9661 submissions, we excluded duplicate records per user, keeping the earliest record per each user. This reduced the dataset to 9569 entries. At this point we had records from 128 countries, out of which 19 had more than 100 submissions. We included the 20th country which had 96 answers for all questions, and filtered out the remaining countries, which led to our final dataset containing 7599 participants.



###**1.3.3 Time frame of the Moral Machine dataset**
For Moral Machine, the survey was posted on 29 April 2020 (all in English). Survey in other languages was added on 31 May 2020. Data collected up to 8 September 2020 was included in the analysis (data collection was happening to a later date).
...
kbl(
  ...
  booktabs = T,
  escape=T,
  col.names = c("Country", "Male (%)", "Age (SD)", "Know COVID patient (%)", "Health", "Smoker100 (%)", "College (%)", "Conservatives (%)", "Religious (%)","White (%)"),
  align=c("l", rep("c",9)),
  ...
  add_footnote(label= "Smoker100 measures whether or not participants smoked at least 100 cigarettes in their entire life")
...
variables=c("full", "no", "Future", "Past", "Years", "Quality", "Prognosis", "First", "Random", "Pay")
dat_bra=data.frame(country=rep("Brazil", 5),var=c("Gender", "Know COVID Patient", "Age", "Smoking100", "Health"))
...
dat_jap=data.frame(country=rep("Japan", 5),var=c("Gender", "Know COVID Patient", "Age", "Smoking100", "Health"))
...
france=merge(Main_Yougov,france, by="id")
dat_fra=data.frame(country=rep("France", 7), var=c("Gender", "Know COVID Patient", "Age", "Smoking100", "Health", "Politics", "Education"))
...
dat_usa=data.frame(country=rep("US",9),var=c("Gender", "Know COVID Patient", "Age", "Smoking100", "Health", "Politics", "Education", "Ethnicity", "Religiousity"))
